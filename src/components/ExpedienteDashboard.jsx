...
// Servicio que conecta con la API en el dominio unificado
const fetchExpediente = async (id) => {
    ...
    if (!response.ok) {
        if (response.status === 404) {
            throw new Error(`El estudiante con ID/NIE "${id}" no fue encontrado en el registro académico.`);
        }
        throw new Error('Error al conectar con la base de datos o fallo en el servidor.');
    }
...
export default function ExpedienteDashboard() {
    ...
    // Resource reactivo de Solid.js: se dispara automáticamente cuando searchId cambia
    const [expediente] = createResource(searchId, fetchExpediente);

    const handleSearch = (e) => {
        e.preventDefault();
        const query = alumnoId().trim();
        if (query !== '') {
            // Actualizar searchId hace que Solid ponga expediente en 'loading' y oculte los datos anteriores
            setSearchId(query);
        }
    };
    ...
    return (
        <div class="space-y-8">
            ...
            <form onSubmit={handleSearch} class="bg-white p-6 rounded-lg border-b-4 border-kepler-gold shadow-md max-w-xl mx-auto flex gap-4">
                <div class="flex-1">
                    <label class="block text-xs font-black uppercase text-slate-500 mb-2 tracking-wider">
                        ID o NIE del Estudiante
                    </label>
                    <input
                        type="number"
                        value={alumnoId()}
                ...
                <button
                    type="submit"
                    class="bg-kepler-red hover:bg-red-800 text-white font-black uppercase text-xs px-6 rounded transition-colors self-end h-12 border-b-4 border-black/20"
                >
                    Consultar
                </button>
            </form>

            {/* Estado 1: Carga (Spinner) */}
            <Show when={expediente.loading}>
                <div class="text-center py-12">
                    ...
                    <p class="text-xs font-black uppercase text-slate-500 tracking-widest">
                        Accediendo al registro escolar...
                    </p>
                </div>
            </Show>

            {/* Estado 2: Error (Muestra cuando da 404 o falla la API) */}
            <Show when={expediente.error}>
                <div class="bg-red-50 border-l-8 border-kepler-red p-6 rounded-lg max-w-2xl mx-auto text-left shadow-sm">
                    <div class="flex items-center gap-3 mb-1">
                        <span class="text-lg">❌</span>
                        <h4 class="text-kepler-red font-black uppercase text-sm">Registro No Encontrado</h4>
                    </div>
                    <p class="text-sm font-medium text-slate-700 pl-8">
                        {expediente.error.message}
                    </p>
                </div>
            </Show>

            {/* Estado 3: Éxito (Renderiza la tarjeta del expediente si se halló el estudiante) */}
            <Show when={!expediente.loading && expediente()}>
                {(item) => (
                    <div class="bg-white rounded-lg border-l-8 border-kepler-red shadow-xl max-w-3xl mx-auto overflow-hidden text-left">
                        ...
                        <div class="bg-black text-white p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b-4 border-kepler-gold">
                            <div>
                                <span class="text-xs font-black text-kepler-gold uppercase tracking-widest block mb-1">
                        ...
                        <div class="p-8 grid md:grid-cols-2 gap-8 font-medium text-slate-700">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <span class={`inline-block mt-1 px-3 py-1 rounded text-xs font-black uppercase tracking-wider ${
                                        item().estado === 'activo' ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-600'
                                    }`}>
                                        ● {item().estado}
                                    </span>
                                </div>
